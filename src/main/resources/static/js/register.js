...
function getNewVerCode(){
	$("#showvercode").attr("src","util/getNewVerCode.do?s="+(new Date()).getTime());
}

function showVerCode() {
	if ($("#vercodebox").attr("class").includes("hidden")) {
...
function doSignUp(){
	...
	sendSignUpInfo(accountId, name, accountPwd)
...
function sendSignUpInfo(accountId, name, accountPwd){
	$.ajax({
		type : "POST",
		dataType : "text",
